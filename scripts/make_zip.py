...
def  main():
  ...
  installer = "\installer\Trippy Installer.exe"
   
  if demo:
    installer = "\installer\Trippy Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Trippy manual.pdf" 
  ]
  ...
  ZIPNAME = "Trippy-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Trippy-v" + FULLVERSIONSTR + "-win-demo.zip"
