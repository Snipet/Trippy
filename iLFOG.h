...
class ILFO : public IControl {

public:
  ILFO(const IRECT& bounds)
    : IControl(bounds) {
  ...
  void Draw(IGraphics& g) override{
    ...
    for (std::vector<Point>::iterator it = points.begin(); it != points.end(); ++it) {
      Point current = *it;
      // We draw the point
      g.DrawCircle(COLOR_GREEN, convertToGraphicX(current.x), convertToGraphicY(current.y), 3, 0, true);

      // The previous point is a valid point, we draw line also
      if (previous.uid > 0) {
        g.DrawLine(COLOR_GREEN,
